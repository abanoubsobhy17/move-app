...
export default function MoviesPage() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      setLoading(true);
      try {
        const fetchedMovies: { [key: number]: any[] } = {};
        for (const category of categories) {
          const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${tmdbApiKey}&with_genres=${category.id}`);
  ...
  return (
    <div className="container mx-auto p-4 bg-gray-900 text-white min-h-screen">
      ...
      {/* Categories and Trending Slider */}
        ...
        <div className="w-full md:w-1/5 p-4 bg-gray-800 rounded-lg">
          ...
          {categories.map((category) => (
            <button 
              ...
              className={`block w-full text-left px-4 py-2 my-1 rounded ${selectedCategory === category.id ? 'bg-blue-500' : 'bg-gray-700'}`}>
              {category.name}
            </button>
          ))}
        </div>
      ...
      {/* Movies by Category */}
      <h2 className="text-2xl font-bold mt-8 mb-4">{categories.find(c => c.id === selectedCategory)?.name} Movies</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {moviesByCategory[selectedCategory]?.map((movie:any) => (
          <div key={movie.id} className="p-2 text-center">
            <Image
              src={`https://image.tmdb.org/t/p/original${movie.poster_path}`}
              alt={movie.title}
